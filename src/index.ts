...
function isBuiltInFunctionOrVariable(name: string) {
  const builtInNames = [
    'if',
    'for',
    'while',
...
function getDeclaredFunctionsAndVariables(fileContent: string) {
  ...
  fileContent.split('\n').forEach(line => {
    ...
    if ((match = functionRegex.exec(line)) !== null) {
      if (!isBuiltInFunctionOrVariable(match[1])) {
  ...
  while ((match = vueMethodsRegex.exec(fileContent)) !== null) {
    ...
    const methodsMatches = methodsContent.match(/([a-zA-Z0-9_]+)\s*\(/g) || [];
    methodsMatches.forEach((method, index) => {
      method = method.trim().replace('(', '');
      if (!isBuiltInFunctionOrVariable(method)) {
...
async function findUnusedFunctionsAndVariables() {
  ...
  for (const filePath of allFiles) {
    ...
    } = getDeclaredFunctionsAndVariables(fileContent);
...
findUnusedFunctionsAndVariables();
